...
**接口描述**:<p>上传模型文件到MinIO，支持glb、stl、obj等格式，自动保存到model/路径</p>
...
**接口描述**:<p>对3D模型进行等比例缩放，然后进行切片，可选择自动打印</p>
...
|&emsp;&emsp;scaleFactor|缩放因子（0 < scaleFactor < 1 为缩小，如0.5表示缩小为50%）||true|number(double)||
