...
import { screenshot as shot } from 'three-platformize/tools/screenshot'
import { initLoader } from './loader.js'

import { useTween } from './useTween'
import { $requestAnimationFrame, $cancelAnimationFrame } from '../lib.js'
...
const { Easing, exec: execTween } = useTween()
...
let frameId
const startRender = () => {
  const { controls, renderer, scene, camera, clock } = getInstance()
...
const centerAndScale = (scaleRatio = 1) => {
  const { group, camera, scene, controls } = getInstance()
  ...
  raf(() => {
    ...
    createPlatformCube(
      scene,
      new THREE.Box3(new THREE.Vector3(-50, -50, 0), new THREE.Vector3(50, 50, 100))
    )
  })
...
const createCenterPlatform = scene => {
  ...
  console.log(
    `Created center platform at (0,0,0) on XZ plane (horizontal), size=${platformSize}mm×${platformSize}mm`
  )
}
...
const createPlatformCube = (scene, boundingBox = null) => {
  ...
  const createLogoPlane = texture => {
    ...
    console.log(
      'Logo loaded and added at position (0, 0, -0.5) on XZ plane (horizontal) at cube bottom'
    )
  }
    ...
    error => {
      ...
      const tryFallback = () => {
        if (fallbackIndex >= fallbackPaths.length) {
          console.error(
            'All logo paths failed. Please check if logo.png exists at /static/images/logo.png'
          )
          return
        }
...
const setEnv = async (url, isSetBg = true, UrlExt = '') => {
  const { scene, loader } = getInstance()
...
const screenshot = async () => {
  const { scene, renderer, camera, helperCanvas } = getInstance()
  ...
  const toTree = list => {
    list.forEach(item => {
      ...
      if (item.children) {
        ...
        nodeMap[parent.id] &&
          nodeMap[parent.id].children.push(createItem(item, nodeMap[parent.id].layer + 1))
      }
    })
  }
...
const getContainerSize = () => {
  const { canvas, container } = getInstance()
  // #ifdef MP
  // 小程序的width和height是原始尺寸*devicePixelRatio的，会导致event计算点击位置出错，所以这里需要除以devicePixelRatio或者使用clientWidth
  const { clientWidth: width, clientHeight: height, _left: left, _top: top } = canvas
...
// 控制器回到默认位置
const controlReset = async ({ duration = 1000, cb = () => {} }) => {
  ...
  const { target, target0, position0 } = controls
  ...
  return new Promise(r => {
    execTween(obj, duration, {
      ...
      onUpdate: () => {
        let { phi, theta, radius, targetx, targety, targetz } = obj
        ...
        sphericalTmp.phi = Math.max(controls.minPolarAngle, Math.min(controls.maxPolarAngle, phi))
        sphericalTmp.makeSafe()

        // sphericalTmp.radius = radius;
        sphericalTmp.radius = Math.max(controls.minDistance, Math.min(controls.maxDistance, radius))
