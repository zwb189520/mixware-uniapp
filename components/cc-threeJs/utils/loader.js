...
import { GLTFLoader, RGBELoader, OBJLoader, FBXLoader, STLLoader } from '../lib.js'
...
// js版本
import { MeshoptDecoder } from './meshopt/meshopt_decoder.asm.module.js'
// wasm版本,仅支持微小程序
...
import { animation, initAnimation } from './animation'
      ...
      try {
        ...
        loader.load(
          ...
          result => {
            ...
            console.log(
              `${type} 模型加载完成，耗时: ${loadDuration.toFixed(2)}ms (${(loadDuration / 1000).toFixed(2)}秒)`
            )
          ...
          err => {
            ...
            const loadDuration = loadEndTime - loadStartTime
            console.error(
              `${type} 模型加载失败，耗时: ${loadDuration.toFixed(2)}ms (${(loadDuration / 1000).toFixed(2)}秒)`
            )
