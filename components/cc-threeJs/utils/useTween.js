...
import { $requestAnimationFrame } from '../lib.js'
...
export const useTween = () => {
  ...
  const exec = async (target, duration, vars) => {
    ...
    for (const prop in vars) {
      ...
      startValues[prop] = typeof target[prop] === 'number' ? target[prop] : parseFloat(target[prop])
    ...
    return new Promise((resolve, reject) => {
      const animate = () => {
        ...
        const easedProgress = vars.ease ? vars.ease(progress) : progress
        for (const prop in changeValues) {
          const newValue = startValues[prop] + changeValues[prop] * easedProgress
