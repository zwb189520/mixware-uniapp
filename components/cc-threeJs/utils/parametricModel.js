...
export class ParametricModelGenerator {
  ...
  async generateKeychain(options = {}) {
    ...
    // 2. 生成3D文字
    try {
      const font = await this.loadFont(this.getFontPath(fontName), fontName)
      ...
      textGeometry.translate(
        -textWidth / 2,
        -textHeight / 2,
        size.depth / 2 + textOptions.depth / 2
      )
  ...
  async updateText(newText, fontName) {
    ...
    // 移除旧的文字mesh
    const textMesh = this.currentModel.children.find(child => child.userData.isText === true)
    if (textMesh) {
