...
import { useThreeJs, traverse, events, animation } from '@/components/cc-threeJs/index.js'
const {
...
export default {
  ...
  methods: {
    ...
    resetModel() {
      ...
      // #ifndef APP
      const { group, controls, camera } = getInstance()
      if (group) {
      ...
      if (typeof controlReset === 'function') {
        // 立即重置视角，不使用动画以确保准确性
        controlReset({ duration: 0 })

        // 再次确保 camera.up 和 position 正确（针对 Z 轴向上场景）
        if (camera && controls) {
          camera.up.set(0, 0, 1)
    ...
    async onload() {
      // 设置环境贴图
      const { backgroundVisible, env } = this.$props
    ...
    async loadModel() {
      ...
      try {
        const { group, loader, scene, controls } = getInstance()
