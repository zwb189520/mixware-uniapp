...
| 参数名称 | 参数说明 | 请求类型    | 是否必须 | 数据类型 | schema |
| -------- | -------- | ----- | -------- | -------- | ------ |
|modelScaleAndSliceDTO|模型缩放并切片打印请求参数|body|true|ModelScaleAndSliceDTO|ModelScaleAndSliceDTO|
...
|&emsp;&emsp;deviceId|设备ID（可选，如果提供则直接发送打印命令）||false|string||
|&emsp;&emsp;autoPrint|是否自动打印（如果为true且提供了deviceId，则切片完成后自动发送打印命令）||false|boolean||
...
|targetId||query|true|integer(int64)||
...
|&emsp;&emsp;postId|帖子ID||false|integer(int64)||
|&emsp;&emsp;parentCommentId|父评论ID（如果是回复评论，则此字段不为空）||false|integer(int64)||
...
|&emsp;&emsp;commentId|评论ID|integer(int64)||
|&emsp;&emsp;postId|帖子ID|integer(int64)||
|&emsp;&emsp;userId|用户ID|string||
|&emsp;&emsp;username|用户名|string||
|&emsp;&emsp;avatarUrl|用户头像URL|string||
|&emsp;&emsp;parentCommentId|父评论ID|integer(int64)||
