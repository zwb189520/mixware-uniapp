                ...
                setTimeout(() => {
                  plus.android.requestPermissions(
                    ...
                    retryResult => {
                      if (
                        retryResult.granted &&
                        retryResult.granted.length === permissions.length
                      ) {
                        resolve()
                      } else {
          ...
          success: () => {
            ...
            setTimeout(async () => {
              ...
              try {
                ...
                // 根据特征值属性选择 writeType
                const writeType = writeCharacteristic.properties.writeNoResponse
                  ? 'writeNoResponse'
                  : 'write'
  ...
  return new Promise((resolve, reject) => {
    ...
    const resultHandler = res => {
      ...
      try {
        ...
        if (
          resultData.includes('#config#success') ||
          resultData.includes('success') ||
          resultData.includes('true')
        ) {
          isResolved = true
          cleanup()
          resolve({ success: true, message: resultData || '配网成功' })
        } else if (
          resultData.includes('#config#fail') ||
          resultData.includes('fail') ||
          resultData.includes('false')
        ) {
          isResolved = true
          cleanup()
