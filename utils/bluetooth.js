...
function writeInChunks(deviceId, serviceId, characteristicId, buffer, writeType, chunkSize) {
      ...
      function doWrite(retryCount) {
        uni.writeBLECharacteristicValue({
          ...
          fail: function(err) {
            ...
            if (err.code === 10007 && retryCount < 3) {
              setTimeout(function() { doWrite(retryCount + 1) }, 500)
            } else {
              reject(err)
            }
          }
  ...
  return new Promise((resolve, reject) => {
    uni.getBLEDeviceCharacteristics({
      ...
      success: (res) => {
        ...
        const writeType = writeCharacteristic.properties.write ? 'write' : 'writeNoResponse'
        console.log('使用写入特征值:', writeCharacteristic.uuid, '属性:', writeCharacteristic.properties, '写入类型:', writeType)
        setTimeout(async () => {
          await new Promise((mtuDone) => {
            uni.setBLEMTU({
              deviceId: deviceId,
              mtu: 512,
              success: (r) => { console.log('MTU设置成功:', r); mtuDone() },
              fail: (e) => { console.log('MTU设置失败或不支持:', e); mtuDone() }
            })
          })
          try {
            ...
            await writeInChunks(deviceId, '0000181A-0000-1000-8000-00805F9B34FB', writeCharacteristic.uuid, serverBuffer, writeType)
            console.log('服务器URL发送成功')
            console.log('等待设备WiFi列表推送完成...')
            await new Promise((done) => {
              ...
              var onData = function(res) {
                if (isDone) return
                try {
                  var s = ab2str(res.value)
                  buf += s
                  console.log('等待期间收到数据:', s)
                  if (buf.indexOf('#endsplit') >= 0) {
                    console.log('WiFi列表推送完成')
                    isDone = true
                    done()
                  }
                } catch(e) {}
              }
              uni.onBLECharacteristicValueChange(onData)
              setTimeout(function() {
                if (!isDone) {
                  console.log('WiFi列表等待超时，继续发送')
                  isDone = true
                  done()
                }
              }, 8000)
            ...
            await writeInChunks(deviceId, '0000181A-0000-1000-8000-00805F9B34FB', writeCharacteristic.uuid, wifiBuffer, writeType)
            ...
            await writeInChunks(deviceId, '0000181A-0000-1000-8000-00805F9B34FB', writeCharacteristic.uuid, triggerBuffer, writeType)
            console.log('配网触发命令发送成功')
            await new Promise(function(r) { setTimeout(r, 300) })
            const endData = '#config#end#end'
            const endBuffer = str2utf8(endData)
            console.log('发送配网结束命令:', endData)
            await writeInChunks(deviceId, '0000181A-0000-1000-8000-00805F9B34FB', writeCharacteristic.uuid, endBuffer, writeType)
