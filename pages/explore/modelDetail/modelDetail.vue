  ...
  methods: {
    ...
    async loadModelDetail(id) {
      ...
      try {
        ...
        const [detailRes, favoriteRes, likeRes] = await Promise.all(promises)
        ...
        const isCollected = favoriteRes.code === 1 && favoriteRes.data && favoriteRes.data.some(item => String(item.modelId) === String(id))
        const isLiked = likeRes && likeRes.code === 1 ? likeRes.data : false
        ...
        this.modelInfo = {
          ...
          author: data.userId ? `用户_${data.userId.substring(data.userId.length - 6)}` : '匿名用户',
