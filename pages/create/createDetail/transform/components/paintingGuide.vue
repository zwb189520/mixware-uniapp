<template>
  <view class="painting-guide">
    <view class="control-header">
      ...
      <button class="add-btn" @click="handleAddMark">{{ texts.addMark }}</button>
    </view>
    ...
    <view class="mark-list">
      ...
      >
        ...
        <view class="mark-info">
          <text class="mark-position">{{ texts.position }}: {{ mark.position }}</text>
          <text class="mark-instructions">{{ mark.instructions }}</text>
        </view>
        <button class="remove-btn" @click="handleRemoveMark(index)">{{ texts.remove }}</button>
      </view>
      
      <view v-if="marks.length === 0" class="empty-state">
        <text class="empty-text">{{ texts.noPaintingGuides }}</text>
      </view>
    </view>
    ...
    <view v-if="showMarkPopup" class="popup-overlay" @tap="closeMarkPopup">
      <view class="mark-picker-popup" @tap.stop>
        <view class="popup-header">
          <text class="popup-title">{{ texts.addPaintingGuide }}</text>
        </view>
        
        <view class="form-container">
          <view class="form-item">
            <text class="form-label">{{ texts.guideColor }}</text>
          ...
          <view class="form-item">
            <text class="form-label">{{ texts.position }}</text>
            <input 
              class="form-input" 
          ...
          <view class="form-item">
            <text class="form-label">{{ texts.instructions }}</text>
            <textarea 
              class="form-textarea" 
        ...
        <view class="popup-actions">
          <button class="cancel-btn" @click="closeMarkPopup">{{ texts.cancel }}</button>
          <button class="confirm-btn" @click="confirmAddMark">{{ texts.confirmAdd }}</button>
        </view>
      </view>
    </view>
  </view>
  ...
  methods: {
    ...
    confirmAddMark() {
      if (!this.formData.position || !this.formData.instructions) {
        uni.showToast({
          title: this.texts.fillRequiredFields,
          icon: 'none'
        })
        return
      }
