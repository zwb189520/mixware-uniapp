...
export default {
  ...
  onUnload() {
    ...
    if(this.platform && this.platform != 'web'){
      // try {
      //   // 1. 移除全局消息监听 (最重要的一步)
      //   if (typeof plus !== 'undefined' && plus.globalEvent && this.plusMessageListener) {
      //       plus.globalEvent.removeEventListener('plusMessage', this.plusMessageListener);
      //       this.plusMessageListener = null;
      //       console.log('已移除 plusMessage 监听');
      //   } 
      // } catch (e) {
      //     console.error('移除 plusMessage 监听失败:', e);
      // }
